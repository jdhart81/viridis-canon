...
async def main() -> int:
    ...
    print(f"[poll_be] project_id = {project_id}", flush=True)
    project = await Project.from_id(project_id)
    print(f"[poll_be] status     = {project.status}", flush=True)
    if str(project.status).lower() in ("completed", "succeeded", "done"):
        OUT_DIR.mkdir(parents=True, exist_ok=True)
        await project.download_to_directory(str(OUT_DIR))
        print(f"[poll_be] downloaded to {OUT_DIR}", flush=True)
        ...
        print(f"[poll_be] FAILED", flush=True)
        return 2
    else:
        print(f"[poll_be] still running — poll again later", flush=True)
    return 0
