...
theorem conjunctive_chain_weakest_first {p : ι → ℝ} (hp0 : ∀ i, 0 < p i)
  ...
  have hprod : ∏ i ∈ S, p i ≤ ∏ i ∈ T, p i :=
    prod_audited_le_of_weakest hp0 hTC (by rw [hScard, hTcard]) hmin
  rw [claimValue_eq_div hp0 hSC, claimValue_eq_div hp0 hTC]
  exact div_le_div_of_nonneg_left (Finset.prod_pos fun i _ => hp0 i).le
    (Finset.prod_pos fun i _ => hp0 i) hprod
...
theorem conjunctive_chain_weakest_first_unique {p : ι → ℝ} (hp0 : ∀ i, 0 < p i)
    ...
    claimValue p C T < claimValue p C S := by
  have hprod : ∏ i ∈ S, p i < ∏ i ∈ T, p i :=
    prod_audited_lt_of_weakest_strict hp0 hTC (by rw [hScard, hTcard]) hne hmin
  rw [claimValue_eq_div hp0 hSC, claimValue_eq_div hp0 hTC]
  exact div_lt_div_of_pos_left (Finset.prod_pos fun i _ => hp0 i)
    (Finset.prod_pos fun i _ => hp0 i) hprod
...
theorem conjunctive_chain_weakest_first_budget {p : ι → ℝ} (hp0 : ∀ i, 0 < p i)
  ...
  have hkC : k ≤ C.card := by
    rw [← hScard]; exact Finset.card_le_card hSC
  have hTC : T ∩ C ⊆ C := Finset.inter_subset_right
  have hTCcard : (T ∩ C).card ≤ k :=
    le_trans (Finset.card_le_card Finset.inter_subset_left) hTcard
  obtain ⟨U, hTU, hUC, hUcard⟩ :=
    Finset.exists_subsuperset_card_eq hTC hTCcard hkC
  calc claimValue p C T = claimValue p C (T ∩ C) := (claimValue_inter p C T).symm
    _ ≤ claimValue p C U := claimValue_mono hp0 hp1 C hTU
    _ ≤ claimValue p C S := conjunctive_chain_weakest_first hp0 hSC hScard hmin hUC hUcard
...
theorem conjunctive_portfolio_monotone {p : ι → ℝ} (hp0 : ∀ i, 0 < p i) (hp1 : ∀ i, p i ≤ 1)
  ...
  exact mul_le_mul_of_nonneg_left (claimValue_mono hp0 hp1 (C c) hST) (hw c hc)
...
theorem conjunctive_portfolio_supermodular {p : ι → ℝ} (hp0 : ∀ i, 0 < p i) (hp1 : ∀ i, p i ≤ 1)
  ...
  refine Finset.sum_le_sum fun c hc => ?_
  rw [← mul_sub, ← mul_sub]
  exact mul_le_mul_of_nonneg_left (claimValue_marginal_mono hp0 hp1 (C c) hST hiT) (hw c hc)
...
theorem conjunctive_portfolio_increasing_marginals {p : ι → ℝ} (hp0 : ∀ i, 0 < p i)
    ...
    portfolioValue p 𝒞 C w (insert i S) - portfolioValue p 𝒞 C w S
      ≤ portfolioValue p 𝒞 C w (insert i T) - portfolioValue p 𝒞 C w T :=
  conjunctive_portfolio_supermodular hp0 hp1 hw hST hiT
...
theorem greedy_first_pick_is_distractor {e : ℝ} (he0 : 0 < e) {x y : Fin 3}
  ...
  obtain ⟨hx, -, -⟩ := h
  have h2 := hx 2
  rw [greedyMarg_empty_c] at h2
  rcases fin3_cases x with rfl | rfl | rfl
  · rw [greedyMarg_empty_a] at h2; nlinarith
  · rw [greedyMarg_empty_b] at h2; nlinarith
  · rfl
...
theorem greedy_improvement {e : ℝ} (he0 : 0 < e) {x y : Fin 3} (h : GreedyRun2 e x y) :
  ...
  have hx : x = 2 := greedy_first_pick_is_distractor he0 h
  subst hx
  obtain ⟨-, hne, -⟩ := h
  have hy : y = 0 ∨ y = 1 := by
    rcases fin3_cases y with rfl | rfl | rfl
    · exact Or.inl rfl
    · exact Or.inr rfl
    · exact absurd rfl hne
  rcases hy with rfl | rfl
  · have : ({(2 : Fin 3), 0} : Finset (Fin 3)) = {0, 2} := by decide
    rw [this, greedyF_ac, greedyF_empty]; ring
  · have : ({(2 : Fin 3), 1} : Finset (Fin 3)) = {1, 2} := by decide
    rw [this, greedyF_bc, greedyF_empty]; ring
...
theorem greedy_family_optimum {e : ℝ} (he : e < 1/3)
  ...
  · rw [greedyF_ab]
  · rw [greedyF_ac]; linarith
  · rw [greedyF_bc]; linarith
...
theorem greedy_family_optimum_budget {e : ℝ} (he0 : 0 < e) (he : e < 1/3)
    {S : Finset (Fin 3)} (hS : S.card ≤ 2) :
    greedyF e S ≤ greedyF e {0, 1} := by
  have hcases : S = ∅ ∨ S = {0} ∨ S = {1} ∨ S = {2} ∨ S = {0, 1} ∨ S = {0, 2} ∨ S = {1, 2} := by
    revert hS; revert S; decide
  rw [greedyF_ab]
  rcases hcases with rfl | rfl | rfl | rfl | rfl | rfl | rfl
  · rw [greedyF_empty]; nlinarith
  · rw [greedyF_a]; nlinarith
  · rw [greedyF_b]; nlinarith
  · rw [greedyF_c]; nlinarith
  · rw [greedyF_ab]
  · rw [greedyF_ac]; linarith
  · rw [greedyF_bc]; linarith
...
theorem greedy_family_optimal_improvement (e : ℝ) :
  ...
  rw [greedyF_ab, greedyF_empty]; ring
...
theorem greedy_improvement_ratio {e : ℝ} (he0 : 0 < e) {x y : Fin 3} (h : GreedyRun2 e x y) :
    (greedyF e {x, y} - greedyF e ∅) / (greedyF e {0, 1} - greedyF e ∅)
      = (3 * e - e ^ 2) / (1 - e ^ 2) := by
  rw [greedy_improvement he0 h, greedy_family_optimal_improvement]

/-- `ρ(ε) = (3ε - ε²)/(1 - ε²) → 0` as `ε → 0⁺`. -/
theorem greedy_ratio_tendsto_zero :
    Filter.Tendsto (fun e : ℝ => (3 * e - e ^ 2) / (1 - e ^ 2))
      (nhdsWithin 0 (Set.Ioi 0)) (nhds 0) := by
  have hcont : ContinuousAt (fun e : ℝ => (3 * e - e ^ 2) / (1 - e ^ 2)) 0 := by
    apply ContinuousAt.div
    · fun_prop
    · fun_prop
    · norm_num
  have h0 : Filter.Tendsto (fun e : ℝ => (3 * e - e ^ 2) / (1 - e ^ 2)) (nhds 0) (nhds 0) := by
    have := hcont.tendsto
    simpa using this
...
theorem marginal_greedy_counterexample_family :
    (∀ e : ℝ, 0 < e → e < 1/3 →
      ...
      ∧ Filter.Tendsto (fun e : ℝ => (3 * e - e ^ 2) / (1 - e ^ 2))
          (nhdsWithin 0 (Set.Ioi 0)) (nhds 0) := by
  refine ⟨fun e he0 he => ⟨greedyP_pos he0, greedyP_le_one he, greedyW_nonneg he0,
    greedyRun2_exists he0, fun x y h =>
      ⟨greedy_first_pick_is_distractor he0 h, greedy_improvement he0 h⟩,
    fun S hS => greedy_family_optimum he hS, greedy_family_optimal_improvement e,
    fun x y h => greedy_improvement_ratio he0 h⟩, greedy_ratio_tendsto_zero⟩
