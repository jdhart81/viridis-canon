...
theorem information_symbiosis_chain_rule (p : Ω → ℝ) (Y : Ω → γ) (X1 : Ω → α) (X2 : Ω → β) :
  ...
  have h1 : entropyBits p (fun ω => (Y ω, X1 ω)) = entropyBits p (fun ω => (X1 ω, Y ω)) :=
    entropyBits_pair_comm (p := p) Y X1
  have h2 : entropyBits p (fun ω => (Y ω, X2 ω)) = entropyBits p (fun ω => (X2 ω, Y ω)) :=
    entropyBits_pair_comm (p := p) Y X2
  have h3 : entropyBits p (fun ω => (Y ω, X1 ω, X2 ω))
      = entropyBits p (fun ω => (X1 ω, X2 ω, Y ω)) :=
    entropyBits_triple_rotate (p := p) X1 X2 Y
  simp only [balanceBits, mutualInfoBits, condMutualInfoBits, h1, h2, h3]
  ring
...
theorem conditional_independence_no_positive_balance {p : Ω → ℝ} (hp : ∀ ω, 0 ≤ p ω)
  ...
  have hchain := information_symbiosis_chain_rule p Y X1 X2
  have hzero := condMutualInfoBits_eq_zero_of_condIndep (f := X1) (g := X2) (h := Y) hp hci
  have heq : balanceBits p Y X1 X2 = -mutualInfoBits p X1 X2 := by
    rw [hchain, hzero]; ring
  refine ⟨heq, ?_⟩
  have hnn := mutualInfoBits_nonneg hp hsum X1 X2
  rw [heq]
  linarith
...
theorem net_symbiosis_cost_criterion (p : Ω → ℝ) (Y : Ω → γ) (X1 : Ω → α) (X2 : Ω → β)
  ...
  have hchain := information_symbiosis_chain_rule p Y X1 X2
  simp only [netBalanceBits, hchain]
...
theorem duplicate_and_xor_extremes :
    balanceBits pdup (fun ω => ω) (fun ω => ω) (fun ω => ω) = -1
      ...
      ∧ balanceBits pfour (fun ω => ω) (fun ω => ω.1) (fun ω => ω.2) = 0 :=
  ⟨balance_duplicate, balance_xor, balance_unique_bits⟩
